...
async function driveSync() {
  ...
  try {
    ...
    if (searchR.status === 401) { _driveHandleUnauth(); return; }
    const found  = await searchR.json();
    ...
    if (uploadR.status === 401) { _driveHandleUnauth(); return; }
    if (!uploadR.ok) { updateDriveStatus('error'); return; }
