    ...
    for (const jpegFile of jpegFiles) {
      try {
        ...
        if (existing) {
          ...
          if (!existing.thumb)   existing.thumb   = await compressImage(dataUrl, 300);
          if (!existing.fbThumb) existing.fbThumb = await compressImage(dataUrl, 80);
        } else {
...
function _showRestoreModal(token, latest, folderId) {
  openModal(`
    <div style="padding:24px 20px 20px;text-align:center">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        <button class="btn btn-primary" onclick="_doRestoreFromDrive('${token}',${JSON.stringify(JSON.stringify(latest))},'${folderId}')">
          🔄 Restaurer
        </button>
...
async function _doRestoreFromDrive(token, latestJson, folderId) {
