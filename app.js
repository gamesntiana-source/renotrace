...
// Échappe les caractères HTML dangereux pour les attributs value="" et le contenu texte
function _esc(s) {
...
// Charge le token Drive persisté indépendamment de db (survit à la perte de db)
function _loadSavedDriveToken() {
  try {
    const saved = JSON.parse(localStorage.getItem('rtp_drive_token') || 'null');
    if (saved && saved.token && saved.expiry > Date.now()) return saved;
  } catch {}
  return null;
}

// Initialise toutes les collections vides de db (sans seeding admin)
...
async function dbLoad() {
  ...
  // ★ Détecter DB vide ou corrompue → tenter restauration automatique Drive
  const isEmpty = !db.users.length && !db.chantiers.length && !db.photos.length;
  if (isEmpty && !window._restoreAttempted) {
    window._restoreAttempted = true;
    window._restoreInProgress = true;
    checkAndOfferDriveRestore(); // fire-and-forget, appellera _finishInit() quand terminé
    return;
  }
...
function dbSave() {
  _encrypt(db).then(enc => {
    try {
      localStorage.setItem(DB_KEY, enc);
    } catch {
      // Quota dépassé avec chiffrement — essai sans chiffrement
      try {
        localStorage.setItem(DB_KEY, JSON.stringify(db));
      } catch {
        // Quota totalement dépassé
        console.warn('[RTP] localStorage quota dépassé');
        toast('⚠ Stockage plein — exportez vos données maintenant pour ne pas les perdre !', 'warning');
      }
    }
  }).catch(() => {
    try {
      ...
      toast('⚠ Stockage plein — exportez vos données maintenant !', 'warning');
    }
  });
  scheduleDriveSync();
...
async function doLogin() {
  ...
  let lockData = {};
  try { lockData = JSON.parse(localStorage.getItem(lockKey) || '{}'); } catch {}
...
function importJsonQuick() {
  if (!can(5)) { toast('Accès réservé à l\'administrateur', 'error'); return; }
  const input = document.createElement('input');
...
// Variante pour le flux de restauration : redirige la fenêtre principale (pas popup)
// → au retour OAuth, init() lira le token depuis le hash et déclenchera checkAndOfferDriveRestore()
function driveConnectForRestore() {
  const clientId = db.settings?.driveClientId;
  if (!clientId) { toast('Configurez d\'abord votre Client ID Google Drive dans Profil', 'warning'); return; }
  const redirect = encodeURIComponent(location.origin + location.pathname);
  const scope = encodeURIComponent('https://www.googleapis.com/auth/drive.file');
  location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${redirect}&response_type=token&scope=${scope}`;
}
function checkDriveToken() {
  ...
  if (hash.includes('access_token')) {
    ...
    // ★ Persister le token indépendamment de db
    const driveExpiry = Date.now() + 3500 * 1000;
    localStorage.setItem('rtp_drive_token', JSON.stringify({ token: driveToken, expiry: driveExpiry, clientId: db.settings?.driveClientId }));
    dbSave();
    location.hash = '';
    ...
    // ★ Fallback : token persisté hors db
    const saved = _loadSavedDriveToken();
    if (saved) {
      driveToken = saved.token;
      db.settings.driveToken = driveToken;
    }
  }
...
// Upload un blob JSON sur Drive (crée ou écrase selon fileId)
async function _driveUploadJson(folderId, fileName, blob, existingFileId = null) {
  ...
  const r = await fetch(
    ...
    { method: existingFileId ? 'PATCH' : 'POST', headers: { 'Authorization': `Bearer ${driveToken}` }, body: fd }
  );
  if (r.status === 401) { _driveHandleUnauth(); throw new Error('DRIVE_401'); }
  return r;
}

// Cherche un fichier dans un dossier Drive → fileId ou null
async function _driveFindFile(folderId, fileName) {
...
async function driveSync() {
  ...
  try {
    ...
    const isLarge = JSON.stringify(db).length > 8_000_000;

    // ── 1. Fichier de compatibilité ascendante ─────────────────────────────
    const legacyBlob = new Blob([JSON.stringify(db)], { type: 'application/json' });
    const legacyId   = await _driveFindFile(rootId, 'renotrace-backup.json');
    const legacyR    = await _driveUploadJson(rootId, 'renotrace-backup.json', legacyBlob, legacyId);
    if (!legacyR.ok) { updateDriveStatus('error'); return; }

    // ── 2. Backup daté (ne pas écraser le même jour s'il existe déjà) ──────
    const datedName = `renotrace-backup-${today}.json`;
    const datedId   = await _driveFindFile(rootId, datedName);
    if (!datedId) {
      // Nouveau backup du jour
      if (isLarge) {
        // Séparer photos dans un fichier dédié
        const { photos: _p, ...dbNoPhotos } = db;
        const mainBlob   = new Blob([JSON.stringify({ ...dbNoPhotos, photos: [] })], { type: 'application/json' });
        const photosBlob = new Blob([JSON.stringify({ photos: db.photos, date: today })], { type: 'application/json' });
        const mainR = await _driveUploadJson(rootId, datedName, mainBlob, null);
        if (!mainR.ok) { updateDriveStatus('error'); return; }
        const photosName = `renotrace-photos-${today}.json`;
        await _driveUploadJson(rootId, photosName, photosBlob, null);
      } else {
        const datedBlob = new Blob([JSON.stringify(db)], { type: 'application/json' });
        const datedR = await _driveUploadJson(rootId, datedName, datedBlob, null);
        if (!datedR.ok) { updateDriveStatus('error'); return; }
      }
    }

    // ── 3. Fichier pointeur renotrace-latest.json ──────────────────────────
    const latestData = { date: today, file: datedName, ts: Date.now(), ...(isLarge ? { hasPhotoFile: true } : {}) };
    const latestBlob = new Blob([JSON.stringify(latestData)], { type: 'application/json' });
    const latestId   = await _driveFindFile(rootId, 'renotrace-latest.json');
    await _driveUploadJson(rootId, 'renotrace-latest.json', latestBlob, latestId);

    // ── 4. Garder au maximum 7 backups datés ──────────────────────────────
    try {
      const listQ = `name contains 'renotrace-backup-' and '${rootId}' in parents and trashed=false`;
      const listR = await fetch(
        `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(listQ)}&fields=files(id,name)&orderBy=name`,
        { headers: { 'Authorization': `Bearer ${driveToken}` } }
      );
      if (listR.ok) {
        const listData = await listR.json();
        const datedFiles = (listData.files || [])
          .filter(f => /^renotrace-backup-\d{4}-\d{2}-\d{2}\.json$/.test(f.name))
          .sort((a, b) => a.name.localeCompare(b.name));
        const toDelete = datedFiles.slice(0, Math.max(0, datedFiles.length - 7));
    ...
    // ── 5. Mettre à jour le token persisté et le statut ───────────────────
    const driveExpiry = Date.now() + 3500 * 1000;
    localStorage.setItem('rtp_drive_token', JSON.stringify({ token: driveToken, expiry: driveExpiry, clientId: db.settings?.driveClientId }));
  ...
  } catch (e) { if (e?.message !== 'DRIVE_401') updateDriveStatus('error'); }
}
...
function exportFullBackup() {
  ...
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `renotrace-backup-${today}.json`;
  a.click(); URL.revokeObjectURL(url);
  // Export toutes les photos
  const total = db.photos.length;
  if (total > 0) {
    toast(`Sauvegarde complète : JSON + ${total} photo(s)…`, 'info');
    db.photos.forEach((p, i) => {
      setTimeout(() => {
        const src = p.data || p.thumb || p.fbThumb;
        if (!src) return;
        const c = db.chantiers.find(x => x.id === p.chantierId);
        const date = p.takenAt ? p.takenAt.slice(0, 10) : 'inconnu';
        const dl = document.createElement('a');
        dl.href = src;
        dl.download = `backup_${(c?.name || 'chantier').replace(/\s+/g, '_')}_${p.surface}_${date}_${i + 1}.jpg`;
  ...
  db.settings.lastAutoBackup = today;
  dbSave();
}
function _showManualBackupToast() {
  const t = document.createElement('div');
  t.id = 'auto-backup-toast';
  t.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#1E293B;color:white;padding:12px 18px;border-radius:12px;z-index:9999;display:flex;align-items:center;gap:12px;box-shadow:0 4px 20px rgba(0,0,0,.4);max-width:340px;width:90%';
  t.innerHTML = `
    <span style="font-size:20px">💾</span>
    <div style="flex:1;font-size:13px;line-height:1.4">
      <div style="font-weight:600">Sauvegarde quotidienne</div>
      <div style="color:#94A3B8;font-size:12px">Aucune sauvegarde effectuée aujourd'hui</div>
    </div>
    <button onclick="exportFullBackup();document.getElementById('auto-backup-toast')?.remove()" style="background:#2563EB;color:white;border:none;border-radius:8px;padding:6px 12px;font-size:12px;cursor:pointer;font-weight:600">Sauvegarder</button>
    <button onclick="document.getElementById('auto-backup-toast')?.remove()" style="background:none;border:none;color:#94A3B8;cursor:pointer;font-size:20px;line-height:1;padding:0 2px">×</button>
  `;
  document.body.appendChild(t);
  setTimeout(() => document.getElementById('auto-backup-toast')?.remove(), 30000);
}

function scheduleAutoBackup() {
  if (!can(5)) return; // réservé à l'administrateur
  const today = new Date().toISOString().slice(0, 10);
  if ((db.settings?.lastAutoBackup || '') === today) return;
  setTimeout(async () => {
    if (!me()) return;
    if (driveToken) {
      await driveSync();
      db.settings.lastAutoBackup = today;
      dbSave();
      toast('💾 Sauvegarde automatique Drive ✓', 'success');
    } else {
      _showManualBackupToast();
    }
  }, 4000);
}
function importJson(file) {
  ...
  r.onload = e => {
    try {
      ...
      if (!data.users || !data.chantiers) { toast('Fichier invalide — format non reconnu', 'error'); return; }
      db = data;
      // Assurer la présence de toutes les collections
      ...
      const chantierCount = db.chantiers.length;
      dbSave();
      // Vérifier que la session courante est toujours valide dans le nouveau DB
      if (session && !db.users.find(u => u.id === session.userId)) {
        session = null;
        sessionStorage.removeItem(SES_KEY);
        toast(`Restauration ✓ — ${chantierCount} chantier(s), ${photoCount} photo(s). Reconnectez-vous.`, 'success');
        navigate('login');
        return;
      }
      toast(`Restauration ✓ — ${chantierCount} chantier(s), ${photoCount} photo(s)`, 'success');
      closeModal();
      navigate('chantiers');
...
function toast(msg, type = 'info') {
  ...
  const _tc = el('toastContainer');
  if (!_tc) { console.warn('[RTP] toast:', msg); return; }
  _tc.appendChild(t);
...
async function driveExportPhotos(chantierId) {
  ...
  try {
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      try {
        const blob  = _dataURItoBlob(src) || await (await fetch(src)).blob();
        ...
        if (r.status === 401) { _driveHandleUnauth(); return; }
        r.ok ? ok++ : fail++;
      } catch { fail++; }
    ...
    if (e?.message !== 'DRIVE_401') toast('Drive : erreur lors de la création du dossier', 'error');
  }
}

// Détecte un token expiré et invalide la session Drive
function _driveHandleUnauth() {
  driveToken = null;
  if (db.settings) db.settings.driveToken = null;
  dbSave();
  toast('Drive : session expirée — reconnectez-vous dans Profil', 'error');
  renderProfile();
}

// Convertit un data URI en Blob sans passer par fetch()
function _dataURItoBlob(src) {
...
async function _driveGetOrCreateFolder(name, parentId) {
  ...
  if (search.status === 401) { _driveHandleUnauth(); throw new Error('DRIVE_401'); }
  ...
  if (create.status === 401) { _driveHandleUnauth(); throw new Error('DRIVE_401'); }
...
function renderProfile() {
  ...
  el('profileBody').innerHTML = `
    ...
    ${can(4) ? `<div class="settings-section">
      <div class="settings-section-title">Configuration</div>
      <div class="settings-row">
        ${can(5) ? `<div class="settings-item" onclick="showDriveSettings()">
          <span class="settings-icon">☁️</span>
        ...
        <div class="settings-item" onclick="showCompanySettings()">
    ...
    ${can(5) ? `<div class="settings-section">
      ...
      <div class="settings-row">
        <div class="settings-item" onclick="exportFullBackup()">
          <span class="settings-icon">🗃️</span>
          <div class="settings-label"><div class="settings-name">Sauvegarde complète</div><div class="settings-desc">JSON + toutes les photos${db.settings?.lastAutoBackup ? ` · Dernière : ${db.settings.lastAutoBackup}` : ''}</div></div>
          <span class="settings-arrow">↓</span>
        </div>
        ...
        <div class="settings-item" onclick="el('importInput').click()">
          <span class="settings-icon">📂</span>
          <div class="settings-label"><div class="settings-name">Restaurer depuis une sauvegarde</div><div class="settings-desc">Chantiers + photos — fichier JSON</div></div>
          <span class="settings-arrow">↑</span>
        </div>
      </div>
    </div>` : ''}
...
function renderAdmin() {
  ...
  el('adminBody').innerHTML = `
    <div class="tabs admin-tabs" style="margin:-16px -16px 16px;padding:0 16px">
      ...
      ${can(5) ? `<button class="tab" onclick="switchAdminTab(this,'at-backup')">Sauvegarde</button>` : ''}
...
function showEditChantier(id) {
  ...
  openModal(`
    ...
    <div class="field"><label>Nom</label><input id="f-name" type="text" value="${_esc(c.name)}"></div>
    <div class="field"><label>Client</label><input id="f-client" type="text" value="${_esc(c.client)}"></div>
    <div class="field"><label>Adresse</label><input id="f-address" type="text" value="${_esc(c.address)}"></div>
    ...
    <div class="field"><label>Notes</label><textarea id="f-notes">${_esc(c.notes)}</textarea></div>
...
function showEditProfile() {
  ...
  openModal(`
    ...
    <div class="field"><label>Nom complet</label><input id="p-name" type="text" value="${_esc(user.name)}"></div>
    <div class="field"><label>Email</label><input id="p-email" type="email" placeholder="votre@email.com" value="${_esc(user.email)}"></div>
    <div class="field"><label>Identifiant</label><input id="p-user" type="text" value="${_esc(user.username)}" autocapitalize="none"></div>
...
function showGroqSettings() {
  openModal(`
    ...
    <div class="field"><label>Clé Groq</label><input id="g-key" type="text" placeholder="gsk_..." value="${_esc(db.settings?.groqKey)}"></div>
...
function showDriveConfig() {
  openModal(`
    ...
    <div class="field"><label>Client ID Google OAuth 2.0</label>
      <input id="d-id" type="text" placeholder="179182751689-xxx.apps.googleusercontent.com" value="${_esc(db.settings?.driveClientId)}" oninput="validateClientIdInput(this)">
...
function showCompanySettings() {
  openModal(`
    ...
    <div class="field"><label>Nom de l'entreprise</label><input id="co-name" type="text" value="${_esc(db.settings.companyName)}"></div>
...
// ═══════════════════════════════════════════
// ═══════════════════════════════════════════
//   RESTAURATION AUTOMATIQUE DEPUIS DRIVE
// ═══════════════════════════════════════════

// Restaure les photos JPEG stockées dans RénoTrace Pro/[chantier]/ sur Drive
// Compatibles avec driveExportPhotos() : format {surface}_{DD-MM-YYYY}_{id4}.jpg
async function _driveRestorePhotos(token, rootFolderId, onProgress) {
  // Lister tous les sous-dossiers (= dossiers chantier)
  const q = `mimeType='application/vnd.google-apps.folder' and '${rootFolderId}' in parents and trashed=false`;
  ...
  const { files: folders } = await listRes.json();
  if (!folders?.length) return 0;

  let totalRestored = 0;

  for (const folder of folders) {
    // Chercher le chantier correspondant au nom de dossier
    const chantier = db.chantiers.find(c =>
      (c.name || '').replace(/[/\\?%*:|"<>]/g, '-') === folder.name
    );
    if (!chantier) continue;

    // Lister les JPEG dans ce dossier
    const fq = `'${folder.id}' in parents and (mimeType='image/jpeg' or name contains '.jpg') and trashed=false`;
    const filesRes = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(fq)}&fields=files(id,name)&pageSize=1000`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!filesRes.ok) continue;
    const { files: jpegFiles } = await filesRes.json();
    if (!jpegFiles?.length) continue;

    for (const jpegFile of jpegFiles) {
      try {
        // Parser le nom : {surface}_{DD-MM-YYYY}_{id4}.jpg
        const nameNoExt = jpegFile.name.replace(/\.jpg$/i, '');
        const parts = nameNoExt.split('_');
        if (parts.length < 3) continue;
        const id4     = parts[parts.length - 1];
        const dateStr = parts[parts.length - 2]; // DD-MM-YYYY
        const surface = parts.slice(0, parts.length - 2).join('_');

        // Vérifier si la photo existe déjà avec données full-res
        const existing = db.photos.find(p =>
          p.chantierId === chantier.id && p.id.endsWith(id4)
        );
        if (existing?.data) continue; // déjà restaurée

        // Télécharger le JPEG
        const dlRes = await fetch(
          `https://www.googleapis.com/drive/v3/files/${jpegFile.id}?alt=media`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        if (existing) {
          // Compléter l'entrée existante
          existing.data = dataUrl;
          if (!existing.thumb)   existing.thumb   = await compressImage(dataUrl, 300).catch(() => dataUrl);
          if (!existing.fbThumb) existing.fbThumb = await compressImage(dataUrl, 80).catch(() => dataUrl);
        } else {
          // Créer une nouvelle entrée photo
          const [dd, mm, yyyy] = dateStr.split('-');
          const takenAt = (yyyy && mm && dd)
            ? new Date(parseInt(yyyy), parseInt(mm) - 1, parseInt(dd), 12, 0, 0).toISOString()
            : new Date().toISOString();
          const thumb   = await compressImage(dataUrl, 300).catch(() => dataUrl);
          const fbThumb = await compressImage(dataUrl, 80).catch(() => dataUrl);
          db.photos.push({
            id: uid(), chantierId: chantier.id,
            data: dataUrl, thumb, fbThumb,
            lat: null, lng: null, accuracy: null, address: '',
            surface, note: '', takenBy: null, takenAt,
          });
        }

        totalRestored++;
        if (onProgress) onProgress(totalRestored);
...
async function checkAndOfferDriveRestore() {
  // 1. Récupérer le token Drive (URL hash OU localStorage séparé)
  let token = null;
  ...
  if (!token) {
    // Pas de token → init normale avec DB vide (admin seeding + écran login)
    if (!db.users.length) {
      const { hash, salt } = await hashPwdSafe('1234');
      db.users.push({
        id: 'sa_001', username: 'admin', name: 'Administrateur',
  ...
  // Token disponible → définir globalement pour _driveGetOrCreateFolder
  driveToken = token;

  // 2. Chercher renotrace-latest.json sur Drive (timeout 8s)
  try {
    const ctrl = new AbortController();
    setTimeout(() => ctrl.abort(), 8000);
    const folderId = await _driveGetOrCreateFolder('RénoTrace Pro', null);
    const q = `name='renotrace-latest.json' and '${folderId}' in parents and trashed=false`;
    ...
    if (!latestFile) throw new Error('not_found');

    // 3. Lire le fichier pointeur
    const latestRes = await fetch(
      `https://www.googleapis.com/drive/v3/files/${latestFile.id}?alt=media`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!latestRes.ok) throw new Error('not_found');
    const latest = await latestRes.json();

    // 4. Afficher le modal de restauration avant l'écran de login
    _showRestoreModal(token, latest, folderId);

  } catch (e) {
    // Drive inaccessible ou aucun backup → init normale
    if (!db.users.length) {
      const { hash, salt } = await hashPwdSafe('1234');
      db.users.push({
        id: 'sa_001', username: 'admin', name: 'Administrateur',
...
function _showRestoreModal(token, latest, folderId) {
  // Stocker les params dans un global pour éviter toute injection HTML via onclick
  window._pendingRestore = { token, latest, folderId };
  openModal(`
    <div style="padding:24px 20px 20px;text-align:center">
      <div style="font-size:40px;margin-bottom:12px">☁️</div>
      <h3 style="margin:0 0 8px;font-size:18px;color:#F1F5F9">Sauvegarde trouvée</h3>
      <p style="margin:0 0 24px;color:#94A3B8;font-size:14px;line-height:1.5">
        Une sauvegarde du <strong style="color:#E2E8F0">${latest.date}</strong> a été trouvée sur votre Google Drive.
      </p>
      <div style="display:flex;flex-direction:column;gap:10px">
        <button class="btn btn-primary" onclick="_doRestoreFromDrive()">
          🔄 Restaurer
        </button>
        <button class="btn" style="background:#1E293B;color:#94A3B8" onclick="closeModal();_finishInit()">
          Commencer vide
        </button>
      </div>
    </div>
...
async function _doRestoreFromDrive() {
  const { token, latest, folderId } = window._pendingRestore || {};
  if (!token || !latest || !folderId) {
    toast('❌ Paramètres de restauration invalides', 'error');
    _finishInit();
    return;
  }
  ...
  try {
    // Chercher le fichier backup daté
    const q = `name='${latest.file}' and '${folderId}' in parents and trashed=false`;
    const res = await fetch(
      `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id)`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    if (!fileId) throw new Error('backup_file_missing');

    const backupRes = await fetch(
      `https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!backupRes.ok) throw new Error('backup_download_failed');
    let restoredDb = await backupRes.json();

    // Si photos dans fichier séparé, les fusionner
    if (latest.hasPhotoFile) {
      const photoFileName = latest.file.replace('backup', 'photos');
      const qp = `name='${photoFileName}' and '${folderId}' in parents and trashed=false`;
      try {
        const pRes = await fetch(
          `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(qp)}&fields=files(id)`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const pFound = await pRes.json();
        if (pFound.files?.[0]?.id) {
          const pData = await (await fetch(
            `https://www.googleapis.com/drive/v3/files/${pFound.files[0].id}?alt=media`,
            { headers: { Authorization: `Bearer ${token}` } }
          )).json();
          restoredDb.photos = pData.photos || [];
        }
      } catch {} // photos non critiques
    }

    // Injecter dans db
    db = restoredDb;
    _normalizeDb();
    db.settings._fbLastRemote = Date.now() + 90000; // bloquer Firebase 90s
    driveToken = token;
    db.settings.driveToken = token;
    localStorage.setItem('rtp_drive_token', JSON.stringify({ token, expiry: Date.now() + 3500000, clientId: db.settings?.driveClientId }));
    await dbSave();

    const pc = db.chantiers?.length || 0;
    const ppJson = db.photos?.length || 0;

    // ★ Restaurer les photos JPEG depuis les sous-dossiers Drive si nécessaire
    // (backup trop grand → photos absentes du JSON, ou exportées manuellement via driveExportPhotos)
    const needPhotoRestore = db.chantiers.length > 0 &&
      (db.photos.length === 0 || db.photos.some(p => !p.data));
    if (needPhotoRestore) {
      toast('📸 Recherche des photos sur Drive…', 'info');
      let photoCount = 0;
      try {
        photoCount = await _driveRestorePhotos(token, folderId, (n) => {
          if (n % 5 === 0) toast(`📸 ${n} photo(s) restaurée(s)…`, 'info');
        });
        if (photoCount > 0) await dbSave();
      } catch {} // non bloquant
      const ppTotal = db.photos?.length || 0;
      toast(`✅ Restauration réussie — ${pc} chantier(s), ${ppTotal} photo(s)`, 'success');
    } else {
      toast(`✅ Restauration réussie — ${pc} chantier(s), ${ppJson} photo(s)`, 'success');
    }
    _finishInit();

  } catch (e) {
    toast('❌ Restauration échouée — ' + (e.message || 'erreur inconnue'), 'error');
    if (!db.users.length) {
      const { hash, salt } = await hashPwdSafe('1234');
      db.users.push({
        id: 'sa_001', username: 'admin', name: 'Administrateur',
...
// Panneau de restauration manuelle depuis l'écran de login
function showRestorePanel() {
  openModal(`
    <div style="padding:24px 20px 20px">
      <h3 style="margin:0 0 8px;font-size:18px;color:#F1F5F9">🔄 Restaurer l'application</h3>
      <p style="margin:0 0 20px;color:#94A3B8;font-size:13px">Choisissez la source de votre sauvegarde</p>
      <div style="display:flex;flex-direction:column;gap:12px;margin-bottom:16px">
        <button class="btn btn-primary" style="text-align:left;padding:14px 16px" onclick="closeModal();driveConnectForRestore()">
          <div style="font-size:15px;font-weight:600">☁️ Depuis Google Drive</div>
          <div style="font-size:12px;font-weight:400;color:rgba(255,255,255,0.7);margin-top:3px">Récupère le backup + les photos JPEG automatiquement</div>
        </button>
        <button class="btn" style="text-align:left;padding:14px 16px;background:#1E293B;border:1px solid #334155" onclick="closeModal();importJsonQuick()">
          <div style="font-size:15px;font-weight:600;color:#E2E8F0">📂 Depuis un fichier JSON</div>
          <div style="font-size:12px;color:#64748B;margin-top:3px">Importez un fichier de sauvegarde téléchargé manuellement</div>
        </button>
      </div>
      <button class="btn" style="width:100%;background:transparent;color:#64748B;font-size:13px" onclick="closeModal()">Annuler</button>
    </div>
  `);
}

// Phase finale d'init (appelée depuis init() ou depuis checkAndOfferDriveRestore())
...
async function init() {
  ...
  await dbLoad();
  // Si DB vide → checkAndOfferDriveRestore() gère la suite (fire-and-forget)
...
function previewDesordrePhotos(input) {
  ...
  Array.from(input.files).slice(0,4).forEach(file => {
    ...
    reader.onerror = () => toast('Erreur lecture image', 'error');
    reader.readAsDataURL(file);
  });
}
